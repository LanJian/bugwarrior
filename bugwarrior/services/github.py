...
class GithubClient(ServiceClient):
    def __init__(self, host, auth, cookies_filename):
        self.host = host
        self.auth = auth
        ...
        #
        # XXX jackhxs 2023-05-21: this is hacky but I don't really care unless I decide
        # to make a PR for this change
        if cookies_filename is not None:
            # firefox has a lock on the cookies database, we are just gonna copy the
            # file to get around that
            cookies_temp = f"{cookies_filename}.temp"
            shutil.copyfile(cookies_filename, cookies_temp)

            conn = sqlite3.connect(cookies_temp)
            cur = conn.cursor()
            data = cur.execute(
                "select name, value from moz_cookies where host = "
                f"'{self.host}' or host = '.{self.host}'"
            ).fetchall()

            cookies = {item[0]: item[1] for item in data}
    ...
    def get_comments(self, username, repo, number):
        url = self._api_url(
            # XXX jackhxs 2023-05-22: changing this to `pulls` since `issues` doesn't
            # seem to include all comments
            "/repos/{username}/{repo}/pulls/{number}/comments?per_page=100",
            username=username,
            repo=repo,
            number=number,
        )
...
class GithubService(IssueService):
    ...
    def __init__(self, *args, **kw):
        ...
        self.client = GithubClient(self.config.host, auth, self.config.cookies_filename)
